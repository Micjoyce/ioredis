...
Redis.prototype.sendCommand = function (command, stream) {
  ...
  if (this.condition.subservice && !_.includes(Command.FLAGS.VALID_IN_SUBSERVICE_MODE, command.name)) {
      command.reject(new Error('Connection in subservice mode, only subservice commands may be used'));
      return command.promise;
  }
