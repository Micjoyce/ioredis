...
Command.prototype.cl_resolve = function (value) {
    var _this = this;
    var result = value;
    var transformer;
    try {
        ...
        _this.callback(err, result);
    }
};

Command.FLAGS = {
  // Commands that can be processed when Redis is loading data from disk
    VALID_WHEN_LOADING: ['info', 'auth', 'select', 'subscribe', 'unsubscribe', 'psubscribe', 'pubsubscribe',
        'subservice', 'psubservice', 'unsubservice', 'punsubservice',
        'publish', 'shutdown', 'replconf', 'role', 'pubsub', 'command', 'latency'],
  ...
  VALID_IN_SUBSERVICE_MODE: ['subservice', 'psubservice', 'unsubservice', 'punsubservice', 'getservice'],
  ENTER_GETSERVICE_MODE: ['getservice','ungetservice'],
  ENTER_SUBSERVICE_MODE: ['subservice', 'psubservice'],
  EXIT_SUBSERVICE_MODE: ['unsubservice', 'punsubservice'],
