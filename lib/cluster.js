...
Cluster.prototype.createNode = function (port, host) {
  ...
  if (!this.nodes[key]) {
    ...
    this.nodes[key].once('end', function () {
      ...
      if (_this.subservice === deadNode) {
          _this.selectSubservice();
      }
...
Cluster.prototype.selectSubService = function () {
    this.subservice = this.selectRandomNode();
    if (this.subservice === null) {
        return;
    }
    // Re-subservice previous channels
    var previousChannels = { subservice: [] , psubservice: [] };
    if (this.lastActiveSubservice && this.lastActiveSubservice.prevCondition) {
        var subservice = this.lastActiveSubservice.prevCondition.subservice;
        if (subservice) {
            previousChannels.subservice = subservice.channels('subservice');
            previousChannels.psubservice = subservice.channels('psubservice');
        }
    }
    if (previousChannels.subservice.length || previousChannels.psubservice.length) {
        var pending = 0;
        _.forEach(['subservice', 'psubservice'], function (type) {
            var channels = previousChannels[type];
            if (channels.length) {
                pending += 1;
                debug('%s %d channels', type, channels.length);
                this.subservice[type](channels).then(function () {
                    if (!--pending) {
                        this.lastActiveSubservice = this.subservice;
                    }
                }.bind(this)).catch(noop);
            }
        }, this);
    } else {
        if (this.subservice.status === 'wait') {
            this.subservice.connect().catch(noop);
        }
        this.lastActiveSubservice = this.subservice;
    }
    _.forEach(['message', 'messageBuffer'], function (event) {
        var _this = this;
        this.subservice.on(event, function (arg1, arg2) {
            _this.emit(event, arg1, arg2);
        });
    }, this);
    _.forEach(['pmessage', 'pmessageBuffer'], function (event) {
        var _this = this;
        this.subservice.on(event, function (arg1, arg2, arg3) {
            _this.emit(event, arg1, arg2, arg3);
        });
    }, this);
};
...
Cluster.prototype.sendCommand = function (command, stream, node) {
  ...
  function tryConnection(random, asking) {
    ...
    if (_this.status === 'ready') {
      ...
      } if (_.includes(Command.FLAGS.ENTER_SUBSERVICE_MODE, command.name) ||
                 _.includes(Command.FLAGS.EXIT_SUBSERVICE_MODE, command.name)) {
          redis = _this.subservice;
      } else {
