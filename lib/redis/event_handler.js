...
exports.readyHandler = function (self) {
  return function () {
    ...
    if (self.prevCondition) {
      ...
      if (condition.subservice && self.options.autoResubservice) {
          // We re-select the previous db first since
          // `SELECT` command is not valid in sub mode.
          if (self.condition.select !== finalSelect) {
              debug('connect to db [%d]', finalSelect);
          ...
          var subserviceChannels = condition.subservice.channels('subservice');
          if (subserviceChannels.length) {
              debug('subservice %d channels', subserviceChannels.length);
              self.subservice(subserviceChannels);
          }
          var psubserviceChannels = condition.subservice.channels('psubservice');
          if (psubserviceChannels.length) {
              debug('psubservice %d channels', psubserviceChannels.length);
              self.psubservice(psubserviceChannels);
          }
      }
  }
